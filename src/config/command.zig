...
/// /bin/sh or equivalent), specify a `!`-prefix. e.g. `!nvim a b c`.
///
...
/// non-shell-expanded case, it is an error if the command can't be
/// found given the inherited PATH.
pub const Command = union(enum) {
    ...
    pub fn parseCLI(
        ...
        // If the input doesn't start with '!' then we're shell
        // expanding the rest and we can consider the whole thing
        // as a single command since it'll be passed into a shell.
        if (trimmed[0] != '!') {
